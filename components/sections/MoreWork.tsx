...
const otherProjects = [
  { title: "Personal CRM", type: "Business Management System", url: "/personal-crm-preview.png" },
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-8">
          ...
          <div className="lg:col-span-7 flex flex-col">
            <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-10 text-black flex items-center gap-4">
            ...
            <div className="flex flex-col border-t border-black/10">
              {otherProjects.map((project, i) => (
                <motion.a
                  ...
                  className="group flex flex-col md:flex-row md:items-center justify-between py-6 md:py-8 border-b border-black/10 hover:bg-black/5 transition-colors px-4 -mx-4 cursor-pointer"
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-8 mb-4 md:mb-0">
                    <h4 className="text-2xl md:text-3xl font-bold uppercase tracking-tight group-hover:text-black transition-colors">
                      {project.title}
                    </h4>
                  </div>
                  <div className="flex items-center gap-6 text-black/50 group-hover:text-black transition-colors">
          ...
          <div className="lg:col-span-5 flex flex-col">
            <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tighter mb-10 text-black flex items-center gap-4">
              The <span className="text-black">Lab</span>
            </h3>
            ...
            <div className="flex flex-col gap-6">
              {inDevelopment.map((project, i) => (
                <motion.div
                  ...
                  key={i}
                  className="bg-white border-2 border-black p-8 relative overflow-hidden group hover:shadow-[8px_8px_0_0_#000] transition-shadow duration-300 cursor-pointer"
                >
                  {/* Flashing Status Dot */}
                  <div className="absolute top-8 right-8 flex items-center gap-2 z-20">
                    ...
                    <span className="text-xs font-mono uppercase tracking-widest text-black font-bold hidden md:block">
                      {project.status}
                    </span>
                  </div>

                  <div className="relative z-20">
                    <div className="text-black/40 mb-6 group-hover:text-black transition-colors">
                      {project.icon}
                    </div>
                    
                    <h4 className="text-2xl md:text-3xl font-black uppercase tracking-tighter mb-2 text-black">
                      {project.title}
                    </h4>
                    <p className="text-xs font-mono uppercase tracking-widest text-black/50 mb-6 border-b border-black/10 pb-6">
                      {project.type}
                    </p>
                    <p className="text-black/70 leading-relaxed text-sm md:text-base group-hover:text-black transition-colors">
                      {project.desc}
                    </p>
                  </div>
                </motion.div>
